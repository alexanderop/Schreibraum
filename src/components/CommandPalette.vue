...
function handleKeydown(e: KeyboardEvent) {
  // Command/Ctrl + O for toggle preview
    ...
    >
      <div class="min-h-screen px-4 text-center">
        ...
        <div
          ...
          @click.stop
        >
          <div class="bg-[#1d1d1d] rounded-lg shadow-2xl overflow-hidden border border-[#323232]">
            <div class="p-3 border-b border-[#323232]">
              <input
                ref="inputRef"
                v-model="searchQuery"
                type="text"
                class="w-full bg-transparent text-white px-2 py-1 text-sm focus:outline-none"
                placeholder="Type > to see available commands..."
              >
            </div>

            <div v-if="showCommands" class="max-h-[440px] overflow-y-auto" role="listbox">
              <div
                v-for="(command, index) in filteredCommands"
                :key="command.id"
                role="option"
                class="px-3 py-2 cursor-pointer text-sm" :class="[
